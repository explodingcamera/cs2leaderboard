...
const fetchLeaderboard = async (region: string): Promise<ResponseData> => {
...
const decodeDataDetails = (data: string) => {
	...
	const uint8array = new Uint8Array(
		data2.match(/.{1,2}/g)?.map((byte) => Number.parseInt(byte, 16)) ?? [],
	);
	const res = ScoreLeaderboardData.decode(uint8array);

	const getTag = <T>(tag: number, fn?: (arg: number) => T) => {
		const val = res.matchentries.find((entry) => entry.tag === tag)?.val;
